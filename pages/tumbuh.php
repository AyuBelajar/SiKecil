...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'tambah_bayi') {
    ...
    if (!$namaBayi || !$tglLahir || !in_array($jk, ['L','P'])) {
        ...
        $ins = $db->prepare('INSERT INTO babies (user_id, nama, tanggal_lahir, jenis_kelamin) VALUES (?,?,?,?)');
        $ins->execute([$user['id'], $namaBayi, $tglLahir, $jk]);
...
<div class="page-wrapper">
  ...
  <!-- ── BAGIAN 1: Tambah Bayi ── -->
  <div class="section-box">
    <h2>👶 Tambah Profil Bayi</h2>
    <form method="POST">
      ...
      <div class="tw-form-row">
        ...
        <div style="display:flex;align-items:flex-end;">
          <button class="form-btn" type="submit" style="margin-bottom:0;">Tambah Bayi</button>
        </div>
      </div>
    </form>

    <?php if (!empty($babies)): ?>
      <div style="margin-top:20px;padding-top:18px;border-top:1px solid #eef4f4;">
        <p style="font-size:.85rem;font-weight:600;color:var(--text-mid);margin-bottom:10px;">Bayi terdaftar – klik untuk lihat riwayat:</p>
