...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'simpan_ukur') {
  ...
  if (!$chkBaby->fetch()) {
    ...
    $whoRef = [
      6 => ['bb' => 7.9, 'tb' => 67.6, 'lk' => 43.3],
      9 => ['bb' => 9.2, 'tb' => 72.3, 'lk' => 44.5],
      12 => ['bb' => 10.2, 'tb' => 76.1, 'lk' => 45.7],
      18 => ['bb' => 11.5, 'tb' => 82.3, 'lk' => 46.9],
      24 => ['bb' => 12.7, 'tb' => 87.8, 'lk' => 47.6],
      36 => ['bb' => 14.3, 'tb' => 95.1, 'lk' => 49.0]
    ];
    $keys = array_keys($whoRef);
    $closest = $keys[0];
    foreach ($keys as $age) {
      if (abs($age - $usia) < abs($closest - $usia))
        $closest = $age;
    }
    $ref = $whoRef[$closest];
...
<div class="page-wrapper">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:8px;">
    ...
    <a href="logout.php"
      style="color:var(--text-mid);font-size:.88rem;font-weight:600;text-decoration:none;padding:8px 18px;border:2px solid #dde9ea;border-radius:50px;transition:all .2s;"
  ...
  <?php showFlash(); ?>
  <?php if (!empty($errors)): ?>
    <div class="alert alert-error"><?php foreach ($errors as $e)
  ...
  <?php else: ?>
    ...
    <div class="section-box" style="max-width:700px;margin-bottom:10px;padding:18px 24px;">
      <p style="font-size:.88rem;font-weight:700;color:var(--text-mid);margin-bottom:10px;">Pilih bayi untuk lihat
        riwayat:</p>
      <?php foreach ($babies as $b): ?>
        <a href="tumbuh.php?baby=<?= $b['id'] ?>" class="baby-card <?= $activeBabyId === (int) $b['id'] ? 'active' : '' ?>">
          <?= $b['jenis_kelamin'] === 'L' ? '👦' : '👧' ?>
          <?= htmlspecialchars($b['nama']) ?>
        </a>
      <?php endforeach; ?>
      <a href="profil.php"
        style="font-size:.82rem;color:var(--text-mid);font-weight:600;text-decoration:none;margin-left:8px;">
        + Tambah bayi baru
      </a>
    </div>
  <?php endif; ?>
  ...
  <?php if (!empty($babies)): ?>
    <div class="section-box">
      ...
      <form method="POST">
        ...
        <textarea class="form-input" name="catatan" placeholder="Contoh: bayi aktif, nafsu makan baik..." rows="2"
          ...
          <tbody>
            <?php foreach ($riwayat as $r): ?>
              <tr>
                ...
                <td style="max-width:160px;font-size:.83rem;color:var(--text-mid);">
                  <?= htmlspecialchars($r['catatan'] ?? '-') ?></td>
                <td>
                  <button class="btn-hapus"
                    onclick="if(confirm('Hapus catatan ini?')) window.location='tumbuh.php?hapus=<?= $r['id'] ?>&baby=<?= $activeBabyId ?>'">🗑️
                    Hapus</button>
                </td>
              </tr>
            <?php endforeach; ?>
  ...
  <?php elseif ($selectedBaby): ?>
    <div class="alert alert-success" style="max-width:700px;">Belum ada catatan untuk
      <?= htmlspecialchars($selectedBaby['nama']) ?>. Tambahkan pengukuran pertama di atas!</div>
  <?php endif; ?>

  <!-- ── BAGIAN 4: Milestones ── -->
  <div style="margin-top:48px;">
    <div class="page-title-bar">
      <h1 style="font-size:1.5rem;">🌟 Milestone Perkembangan</h1>
    </div>
  ...
  <div style="margin-top:28px;">
    <a href="../index.php" style="color:var(--teal);font-size:.9rem;font-weight:600;text-decoration:none;">← Kembali ke
      Beranda</a>
  </div>
</div>
